...
# Hardcoded research query (replaces argparse)
QUERY = "MiMo V2.5 omnimodal multimodal audio transcription API OpenCode Go payload format audio_url long audio chunking 2026"
...
def search_node(state: dict) -> dict:
    ...
    if len(query) > 100:
        ...
        important = [w for w in keywords if any(k in w.lower() for k in ['mimo', 'omnimodal', 'multimodal', 'audio', 'transcription', 'opencode', 'chunking', 'asr', 'xiaomi', 'api', 'ffmpeg', 'streamlit'])]
        if len(important) >= 3:
...
print("\nResults saved to /home/z/my-project/download/agent_output.json")
